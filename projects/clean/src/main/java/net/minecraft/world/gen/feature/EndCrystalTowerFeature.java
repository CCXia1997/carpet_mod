...
public class EndCrystalTowerFeature extends Feature<NoFeatureConfig>
{
    ...
    public boolean place(IWorld worldIn, IChunkGenerator <? extends IChunkGenSettings > generator, Random rand, BlockPos pos, NoFeatureConfig config)
    {
        ...
        else
        {
            ...
            for (BlockPos.MutableBlockPos blockpos$mutableblockpos : BlockPos.getAllInBoxMutable(new BlockPos(pos.getX() - i, 0, pos.getZ() - i), new BlockPos(pos.getX() + i, this.spike.getHeight() + 10, pos.getZ() + i)))
            {
                if (blockpos$mutableblockpos.distanceSq((double)pos.getX(), (double)blockpos$mutableblockpos.getY(), (double)pos.getZ()) <= (double)(i * i + 1) && blockpos$mutableblockpos.getY() < this.spike.getHeight())
                {
                    this.setBlockState(worldIn, blockpos$mutableblockpos, Blocks.OBSIDIAN.getDefaultState());
                }
                    ...
                    for (int l = -2; l <= 2; ++l)
                    {
                        for (int i1 = 0; i1 <= 3; ++i1)
                        {
                            ...
                            if (flag || flag1 || flag2)
                            {
                                ...
                                this.setBlockState(worldIn, blockpos$mutableblockpos1.setPos(pos.getX() + k, this.spike.getHeight() + i1, pos.getZ() + l), iblockstate);
                            }
                        }
                    }
                }
            ...
            EntityEnderCrystal entityendercrystal = new EntityEnderCrystal(worldIn.getWorld());
            entityendercrystal.setBeamTarget(this.beamTarget);
            entityendercrystal.setInvulnerable(this.crystalInvulnerable);
            entityendercrystal.setLocationAndAngles((double)((float)pos.getX() + 0.5F), (double)(this.spike.getHeight() + 1), (double)((float)pos.getZ() + 0.5F), rand.nextFloat() * 360.0F, 0.0F);
            worldIn.spawnEntity(entityendercrystal);
            this.setBlockState(worldIn, new BlockPos(pos.getX(), this.spike.getHeight(), pos.getZ()), Blocks.BEDROCK.getDefaultState());
            return true;
        }
    }
