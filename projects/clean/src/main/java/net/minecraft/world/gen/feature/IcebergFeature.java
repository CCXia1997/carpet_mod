...
{
    public boolean place(IWorld worldIn, IChunkGenerator <? extends IChunkGenSettings > generator, Random rand, BlockPos pos, IcebergConfig config)
    {
        pos = new BlockPos(pos.getX(), worldIn.getSeaLevel(), pos.getZ());
        boolean flag = rand.nextDouble() > 0.7D;
        IBlockState iblockstate = config.state;
        ...
        for (int l1 = -k1; l1 < k1; ++l1)
        {
            for (int i2 = -k1; i2 < k1; ++i2)
            {
                for (int j2 = 0; j2 < l; ++j2)
                {
                    ...
                    if (flag1 || l1 < k2)
                    {
                        this.func_205181_a(worldIn, rand, pos, l, l1, j2, i2, k2, k1, flag1, j, d0, flag, iblockstate);
                    }
                }
            }
        }
        ...
        for (int i3 = -k1; i3 < k1; ++i3)
        {
            for (int j3 = -k1; j3 < k1; ++j3)
            {
                for (int k3 = -1; k3 > -i1; --k3)
                {
                    ...
                    if (i3 < l2)
                    {
                        this.func_205181_a(worldIn, rand, pos, i1, i3, k3, j3, l2, l3, flag1, j, d0, flag, iblockstate);
                    }
                }
            }
        }
