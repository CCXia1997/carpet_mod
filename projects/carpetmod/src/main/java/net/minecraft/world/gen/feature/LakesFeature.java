...
public class LakesFeature extends Feature<LakesConfig>
{
    ...
    public boolean place(IWorld worldIn, IChunkGenerator <? extends IChunkGenSettings > generator, Random rand, BlockPos pos, LakesConfig config)
    {
        ...
        else
        {
                ...
                for (int l2 = 0; l2 < 16; ++l2)
                {
                    for (int k = 0; k < 8; ++k)
                    {
                        ...
                        if (flag)
                        {
                            ...
                            if (k < 4 && !material.isSolid() && worldIn.getBlockState(pos.add(k1, k, l2)).getBlock() != config.block)
                            {
                                return false;
                            }
                        }
            ...
            for (int l1 = 0; l1 < 16; ++l1)
            {
                for (int i3 = 0; i3 < 16; ++i3)
                {
                    for (int i4 = 0; i4 < 8; ++i4)
                    {
                        ...
                        {
                            worldIn.setBlockState(pos.add(l1, i4, i3), i4 >= 4 ? field_205188_a : config.block.getDefaultState(), 2);
                        }
                    }
                }
            }
                ...
                for (int j3 = 0; j3 < 16; ++j3)
                {
                    for (int j4 = 4; j4 < 8; ++j4)
                    {
                        if (aboolean[(i2 * 16 + j3) * 8 + j4])
                        {
                            ...
                            if (Block.isDirt(worldIn.getBlockState(blockpos).getBlock()) && worldIn.getLightFor(EnumLightType.SKY, pos.add(i2, j4, j3)) > 0)
                            {
            ...
            if (config.block.getDefaultState().getMaterial() == Material.LAVA)
            {
            ...
            if (config.block.getDefaultState().getMaterial() == Material.WATER)
            {
